#! /bin/bash
#####################################################################################################################
# Job Name: fxit_split_by_version_pr.sh (THIS VERSION HAS A HARDCODED ENV OF P)
#
...
do
	...
	if [ -f $file ]; then
		...
		do
			# only process files if they exist (this will prevent from creating files with headers and trailers only)
			if [ -e "$split_file" ]; then
				#define temp file
				export temp_file=$split_file.temp
				# use sed to insert the header record before the first line (read split file and write to temp file)
				printf '%s\n' "$header" | cat - $split_file > $temp_file
				# move temp file to split file
				mv $temp_file $split_file
				# add the trailer to the end of the file
				echo "$trailer" >> $split_file
				# copy split file to archive/version folder so we can save a copy for ourselves
				cp $split_file /disk/data/websvc/ftp/dropzone/ftpfxit${env}/xmlt/stage/archive/version
		...
		# move unsplit file to archive/original folder so we don't process it again next time
		mv $file /disk/data/websvc/ftp/dropzone/ftpfxit${env}/xmlt/stage/archive/original
